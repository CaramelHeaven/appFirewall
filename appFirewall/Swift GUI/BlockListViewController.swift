...
class BlockListViewController: appViewController {
    ...
    override func getTableCell(tableView: NSTableView, tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        if cellIdentifier == "ButtonCell" {
            guard let cell = tableView.makeView(withIdentifier: cellId, owner: self) as? blButton else {
                print("WARNING: problem in blocklistView making button cell")
                return nil
            }
            //			cell.title = "Allow"
            //			cell.bl_item = item?.pointee
            //			cell.tag = row
            cell.action = #selector(Click)
